...
class Config(object):
    def __init__(self):
        ...
        # demo image
        parser.add_argument('--input', type=str, default='./dataset/demo/input')
        parser.add_argument('--output', type=str, default='./dataset/demo/output')
        ## dataset parameter
...
# load one image
images = glob.glob(cfg.input+'/*.png')
import time
tic_start = time.time()
for image in images:
    ...
    img.save(cfg.output+'/'+filename)
    print 'time cost: %.1fms'%((time.time()-tic_start)*1000)
tic_end = time.time()
print 'average time: %.1fms per image'%((tic_end-tic_start)/len(images)*1000)
